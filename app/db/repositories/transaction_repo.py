...
class TransactionRepository:
    ...
    async def _ensure_account(self, user_id: int, account_id: int) -> Account:
        q = select(Account).where(Account.id == account_id, Account.user_id == user_id, Account.archived == False)
        acc = (await self.session.execute(q)).scalar_one_or_none()
        if not acc:
            raise NotFound("account")
        return acc

    async def _ensure_category(self, user_id: int, category_id: int | None) -> Category | None:
    ...
    async def get(self, user_id: int, tx_id: int) -> Transaction:
        q = select(Transaction).where(Transaction.id == tx_id, Transaction.user_id == user_id)
    ...
    async def list(
        ...
        *,
        ...
        q = q.order_by(desc(Transaction.created_at), desc(Transaction.id)).limit(limit + 1)
    ...
    async def create(
        ...
        *,
        ...
        if self.enforce_non_negative and direction == Direction.outgoing:
            stmt = (
                update(Account)
                .where(Account.id == acc.id, Account.user_id == user_id, Account.balance >= amount)
            ...
            stmt = update(Account).where(Account.id == acc.id, Account.user_id == user_id).values(
    ...
    async def update(
        ...
        if cat:
            need_kind = CategoryKind.expense if old_direction == Direction.outgoing else CategoryKind.income
        ...
        if new_account_id == old_account_id:
            ...
            delta_old = -old_amount if old_direction == Direction.incoming else +old_amount
            await self._apply_balance_delta(user_id, old_account_id, delta_old)
            delta_new = +new_amount if old_direction == Direction.incoming else -new_amount
    ...
    async def delete(self, user_id: int, tx_id: int) -> None:
        ...
        stmt = update(Account).where(Account.id == tx.account_id, Account.user_id == user_id).values(
            balance=Account.balance + delta
        )
        await self.session.execute(stmt)
